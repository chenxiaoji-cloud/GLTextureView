 ...
 * Created on 15-10-18.
 */
public class SampleGLTextureView extends TextureView implements TextureView.SurfaceTextureListener {
    ...
    public SampleGLTextureView(Context context) {
        super(context);
        initialize(context);
    }

    public SampleGLTextureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialize(context);
    }

    public SampleGLTextureView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void startThread(SurfaceTexture surface, int width, int height, float targetFramesPerSecond) {
        Log.d(TAG, "Starting SampleGLTextureView thread");
    ...
    public synchronized void setPaused(boolean isPaused) {
        Log.d(TAG, String.format("Setting SampleGLTextureView paused to %s", isPaused));
        paused = isPaused;
    }
    ...
    public void stopThread() {
        if (thread != null) {
            Log.d(TAG, "Stopping and joining SampleGLTextureView");
            isRunning = false;
            try {
                thread.join();
